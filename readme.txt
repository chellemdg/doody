=== Doody ===
Contributors: chellemdg
Requires at least: WordPress 5.0
Tested up to: WordPress 5.1-trunk
Requires PHP: 7.2.12
Stable tag: 1.0.6
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: blog, custom-logo, grid-layout, one-column, two-columns, custom-background, custom-colors, custom-header, custom-menu, featured-image-header, featured-images, flexible-header, full-width-template, threaded-comments, left-sidebar, right-sidebar

== Description ==

Doody is a minimalistic theme  based in “_s” (by Automattic, the main people behind WordPress development) and Twitter Bootstrap (the most popular HTML, CSS, and JS framework for developing responsive, mobile first projects on the web).This theme, like WordPress, is licensed under the GPL.  Other features – Currently using Bootstrap v4.3.1, it has customizable color adjustments with beautiful color schemes by default, uses a harmonious fluid grid system prepared for mobile, you are allowed to remove the link at the footer and it have an impeccable care in every detail. Doody will make your WordPress look beautiful and simple in any situation.

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Doody in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.
4. Navigate to Appearance > Customize in your admin panel and customize to taste.

== Copyright ==

Doody WordPress Theme, Copyright 2019 chellemdg
Doody is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

Doody bundles the following third-party resources:


== Frequently Asked Questions ==

= Does this theme support any plugins? =

Doody includes support for Infinite Scroll in Jetpack.

== Credits ==

_s, Copyright 2015-2018 Automattic, Inc.
**License:** GPLv2 or later
Source: https://github.com/Automattic/_s/

normalize.css, Copyright 2012-2016 Nicolas Gallagher and Jonathan Neal
**License:** MIT
Source: https://necolas.github.io/normalize.css/

Bootstrap, Copyright (c) 2011-2019 Twitter, Inc.
**License:** MIT
Source: https://github.com/twbs/bootstrap

Popper.js, Copyright (c) 2016 Federico Zivolo and contributors
**License:** MIT
Source: https://github.com/FezVrasta/popper.js

WP Bootstrap Navwalker
**License:** GPLv3
Source: https://github.com/wp-bootstrap/wp-bootstrap-navwalker

Bundled header image, Copyright Kaboompics.com
License: CC0 1.0 Universal (CC0 1.0)
Source: https://www.pexels.com/photo/white-desk-with-a-cup-of-coffee-and-mobile-phone-6391/

== Changelog ==

= 1.0.5 - Jun 13 2019 =

* Added new footers menu.
* Added fontawesome icons.
* Added minified version of flaty.min.css.
* Organized css folder.
* Changed fonts.


= 1.0.4 - Jun 05 2019 =

* Add pagination buttons.
* Changed widgets style.
* Fixed responsive issues.
* Updated tags list with the new tags.

= 1.0.3 - Jun 03 2019 =

* Replaced screenshot.
* Changed background color.
* Changed footer credits.
* Added new templates page.


= 1.0.2 - May 17 2019 =

* Added requires PHP in readme.
* Changed version for stable tag in readme.
* Changed author for contributors in readme.
* Changed copyright for credits in readme.
* Added license for popper.js.
* Removed the languages folder.
* Removed inline CSS code in theme files.
* Writted "WordPress" correctly.
* Changed author in style.css.
* Changed license in style.css.

= 1.0.1 - April 17 2019 =

* Added the original files of hover.css, bootstrap.js ,bootstrap.bundle.js, popper.js.
* Added minified version of bootstrap.bundle.min.js,skip-link-focus-fix.min.js.
* Changed credits for copyright in readme.
* Added missing credits in .

= 1.0 - April 02 2019 =

* Initial release

